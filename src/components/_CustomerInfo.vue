...
export default {
  ...
  methods: {
    async init() {
      ...
      await this.getAreaIngo(this.AddCustomerData.city);
    },
    backHistory() {
      this.$router.go(-1);
    },
    ...
    getCustomer() {
      ...
      params.append('customerId', this.$route.params.id);

      return apiPostGetCustomer(params, this.token)
        .then((res) => {
          if (res.status === 200 && res.data.status_code === 'SUCCESS') {
            this.AddCustomerData.shopName = res.data.customer_name;
            this.AddCustomerData.phone = res.data.contact_phone_no;
            this.AddCustomerData.contactPerson = res.data.contact_window;
            this.AddCustomerData.city = res.data.city;
            this.AddCustomerData.area = res.data.area;
            this.AddCustomerData.address = res.data.address;
            this.AddCustomerData.taxIDnumber = res.data.uniform_number;
            this.AddCustomerData.label = res.data.label;
            this.AddCustomerData.lineID = res.data.line_id;
          }
        })
        .catch((error) => {
          console.log(error);
    ...
    modifyCustomer() {
      if (this.$refs.form.validate()) {
        ...
        apiPostModifyCustomer(params, this.token)
          .then((res) => {
            if (res.status === 200 && res.data.status_code === 'SUCCESS') {
              this.dialog = true;
              this.dialogText = '修改客戶成功';
